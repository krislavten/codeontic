...
- ecf4958: `report` / `drift-report`: `--format github` 现在会在**读数没跑成**时往 stdout 打一条 `::error` 注解。
...
- cf54da7: 新增 `codeontic gate` —— CI 门禁入口，把接入仓过去手写在 workflow 里的那套逻辑收进引擎：跑确定性检查、（可选）对基线打分、只为**本次改动引入的** error 判红、渲染裁决、返回退出码。
...
- cf54da7: 新增 `codeontic report` 与 `codeontic drift-report`，与 `gate` 一起构成完整的 CI 面：接入仓不必再把这套逻辑手写进 workflow。
...
- cf54da7: CI 面收编过程中连带的几处跨命令变更与既有缺陷修复。单独列出来，因为它们不属于任何一条新命令，但会改变现有接入方看到的行为。
...
- 4efd541: 新增 CLI `search` 命令：`codeontic search "<query>" [dir]`，模型自由文本 IDF 检索，`model_search` MCP 工具的 CLI 孪生。此前入口检索只存在于 MCP——经 shell/AGENTS.md 通用面接入的 agent 有全部查询命令、唯独没有找入口的那条，只能手工 grep 节点标题。行为与 MCP 完全一致（同一 `runSearchCommand`）：计分命中 + 1-hop 关联、side-channel 落盘带 staleness 戳、模型有 parse error 时拒绝检索而非静默搜残图。多词查询需引号，未加引号时报可复制粘贴的纠正示例。
